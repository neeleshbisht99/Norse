...
namespace NorseNet {
    ...
    Norse::Norse(uint32_t capacity, uint32_t inputDim, uint32_t outputDim, const std::string &mode) {
    ...
    void Norse::preheat(uint32_t num_of_nodes, uint32_t num_of_connections, uint32_t depth) {
        if (args->preheatFlag) {
        ...
        for (uint32_t i = 0; i < args->cpuNum; ++i) {
            threads.emplace_back(
                    std::thread(&Norse::preheatCore, this, start[i], end[i], num_of_nodes, num_of_connections));
        }
    ...
    void Norse::evolve(std::vector<std::vector<float> > &input, std::vector<std::vector<float> > &desire) {
        if (args->evolveFlag) {
    ...
    void Norse::fit(std::vector<std::vector<float> > &input, std::vector<std::vector<float> > &desire) {
        if (capacity > 1) {
            fitAll(input, desire);
        } else {
            if (input.size() != desire.size()) {
    ...
    void Norse::enrich() {
        if (capacity > 1) {
            ...
            std::sort(sortList.begin(), sortList.end(), NorseUtils::compareFitness);
    ...
    std::vector<float> Norse::compute(std::vector<float> &input) {
    ...
    std::vector<std::vector<float> > Norse::computeBatch(std::vector<std::vector<float> > &input) {
        ...
        std::vector<std::thread> threads;
        for (uint32_t i = 0; i < args->cpuNum; ++i) {
            threads.emplace_back(std::thread(&Norse::computeBatchCore, this, start[i], end[i], output));
        }
        for (auto &thread : threads) {
            thread.join();
        }
    ...
    void Norse::resize(uint32_t num) {
        if (num < 1) {
        ...
        if (temp1 < temp2) {
            ...
            std::vector<std::thread> threads;
            for (uint32_t i = 0; i < args->cpuNum; ++i) {
                threads.emplace_back(std::thread(&Norse::mutateCore, this, start[i], end[i]));
            }
            for (auto &thread : threads) {
                thread.join();
            }
    ...
    void Norse::setMutateArgs(std::vector<float> &p) {
    ...
    void Norse::setMutateOdds(float odds) {
        if (odds < 0) {
    ...
    void Norse::setCpuCores(uint32_t num) {
        args->cpuNum = num;
    }

    // Set the learning rate.
    void Norse::setLR(float lr) {
        args->lr = lr;
    }

    // Returns the size of the best individual.
    uint32_t Norse::getSize() {
        checkFirstRun();
    ...
    uint32_t Norse::getInputDim() {
        return inputNum;
    }

    // Returns the output dimension.
    uint32_t Norse::getOutputDim() {
        return outputNum;
    }

    // Returns capacity of Norse.
    uint32_t Norse::getCapacity() {
        return capacity;
    }
    ...
    // Returns mode of Norse (classify or predict).
    std::string Norse::getMode() {
        return args->mode;
    }

    // Returns the number of nodes in each layer of the neural network.
    std::vector<uint32_t> Norse::getLayers() {
        if (args->preheatFlag) {
     ...
     */
    std::vector<float> Norse::getHiddenLayer(uint32_t pos) {
        if (args->preheatFlag) {
            std::cout << "The preheat() is not executed." << std::endl;
            exit(11);
        }
    ...
    // Fit all neural networks in the neural network cluster.
    void Norse::fitAll(std::vector<std::vector<float> > &input, std::vector<std::vector<float> > &desire) {
        if (input.size() != desire.size()) {
            std::cout << "The input data and the desire data do not match!" << std::endl;
            exit(7);
        }
    ...
    void Norse::mutateCore(uint32_t start, uint32_t end) {
        for (uint32_t i = start; i < end; ++i) {
            tempList[i] = args->utils->mutateIndividual(
                    *((*individualList)[args->utils->NorseRandomUint32_t(oldLength)]));
        }
    }

    // BP_Single_Thread() is invoked in every individual of the neural network cluster.
    void Norse::backPropagation() {
        auto start = new uint32_t[args->cpuNum];
        auto end = new uint32_t[args->cpuNum];
        auto individualLength = uint32_t(individualList->size());
        auto part = individualLength / args->cpuNum;
        ...
        std::vector<std::thread> threads;
        for (uint32_t i = 0; i < args->cpuNum; ++i) {
            threads.emplace_back(std::thread(&Norse::backPropagationCore, this, start[i], end[i]));
        }
        for (auto &thread : threads) {
            thread.join();
        }
    ...
    void Norse::backPropagationCore(uint32_t start, uint32_t end) {
        for (uint32_t i = start; i < end; ++i) {
    ...
    void Norse::preheatCore(uint32_t start, uint32_t end, uint32_t num_of_nodes, uint32_t num_of_connections) {
        for (uint32_t i = start; i < end; ++i) {
            NorseNet::NorseUtils::addHiddenNodes(*((*individualList)[i]), num_of_nodes);
            args->utils->addConnections(*((*individualList)[i]), num_of_connections);
        }
    }

    // The functor of computeBatch().
    void Norse::computeBatchCore(uint32_t start, uint32_t end, float **output) {
        ...
        for (uint32_t i = start; i < end; ++i) {
            ...
            if (args->mode == "classify") {
                NorseUtils::softmax(output[i], outputNum);
            }
        }
    ...
    Norse *loadModel(const std::string &path, uint32_t capacity) {
        ...
        auto data = NorseUtils::split(str);
        ...
        auto radiata = new Norse(capacity, source->inputNum, source->outputNum, mode);
    ...
    Norse *loadViaString(const std::string &model, uint32_t capacity) {
        auto data = NorseUtils::split(model);
        auto source = new Individual();
        source->inputNum = uint32_t(std::stoul(data[0]));
        source->outputNum = uint32_t(std::stoul(data[1]));
        source->innovationNum = uint32_t(std::stoul(data[2]));
